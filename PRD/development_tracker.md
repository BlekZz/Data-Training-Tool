...
| # | 任務 | 狀態 | 依賴 | 驗證標準 |
|---|------|------|------|---------|
| 4.1 | 固定錨點 (Z-Anchors) 設計 | ✅ | — | Z2~Z5 儲存 SOP 與清洗區座標 |
| 4.2 | 讀取使用者填寫的 SOP 資料 | ✅ | 4.1 | 依據錨點讀取，無視文字變動 |
| 4.3 | 固定 SOP 列數 (7 列) 與下拉選單 | ✅ | — | 錯誤類型 & 處置策略下拉選單 |
| 4.4 | 讀取使用者貼上的清洗後資料 | ✅ | 4.1 | 依據錨點讀取，支援 20 列範圍 |
| 4.5 | 提交前的 `SubmitLoadingDialog.html` | ✅ | — | 使用 Client-Triggered 模式確保彈窗必關 |
| 4.6 | POST 至 Backend `submit-response` | ✅ | — | 程式碼已實作 (待端對端測試) |
| 4.7 | 渲染 AI Feedback 區塊 | ✅ | — | `renderFeedback` 實作完成 |
| 4.8 | **端對端提交測試** | **⬜** | 4.7 | **待進行：填寫 → 提交 → 看到分數** |
...
| 預設模型 | `gemini-2.0-flash` | 在使用者 API Key 下穩定可用 |
| Fallback 模型 | `gemini-2.0-flash` | 作為主模型忙碌時的備援 |
| Request 格式 | Flat JSON | 比 nested envelope 更簡潔，適合 GAS 環境 |
| Auth 方式 | OAuth Token Header | 企業帳號需要，個人帳號可免 |
